		...
		<div class="col-sm-12 col-md-6">
			...
			<table class="table">
				...
				<tbody v-for="item in basket">
					<tr>
						<td>
							<button class="btn btn-sm" type="button" @click="decreaseQauntity(item)">-</button>
							<span>{{ item.quantity }}</span>
						   <button class="btn btn-sm" type="button" @click="increaseQauntity(item)">+</button>	
						</td>
...
export default {
	...
	methods: {
		...
		increaseQauntity(item) {
		item.quantity++;
	},
	decreaseQauntity(item){
		item.quantity--;
		if(item.quantity === 0) {
